...
begin
  ...
  perd.Visible := false;
  LetraDigite.Enabled := false;
  ShowMessage('Digite uma palavra antes de começar');
  ...
  perd.Visible := false;
  LetraDigite.Enabled := false;
  ShowMessage('Digite uma dica antes de começar');}

  // Inicio da Validação dos campos: Palavra e Dica, para que não passe em branco/nil;
  try
    if length(Word.Text) = 0 then
      abort
  except
  ...
  Gerada.Caption := '';
  Gerada.Visible := true;
  // Concatenação (usar _ ) para determinar a palavra.
...
begin
  ...
  begin
    ...
    if erro = 6 then
    begin
      ShowMessage('Você perdeu, mamaco.');
      Form1.Close;
    end;
    LetraDigite.Text := '';
    Gerada.Caption := b;
